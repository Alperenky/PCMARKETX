...
function loadFeaturedProducts() {
    ...
    // API'den featured ürünleri çekmek için fetch isteği
    fetch('/api/products/featured')
        ...
        .then(data => {
            console.log('Popüler ürünler başarıyla yüklendi, adet:', data.length);
            displayFeaturedProducts(data);
            
            // Butonların bağlanması için kısa bir gecikme ekle
            setTimeout(() => {
                setupFavoriteButtons();
                console.log('Popüler ürünler için favori butonları bağlandı.');
            }, 100);
        })
        .catch(error => {
            ...
            if (productGrid) {
                productGrid.innerHTML = '<div class="no-products">Şu anda öne çıkan ürünler yüklenemedi. Lütfen daha sonra tekrar deneyin.</div>';
            }
        });
}

// Popüler ürünleri görüntüleme fonksiyonu
function displayFeaturedProducts(products) {
    ...
    if (!products || products.length === 0) {
        productGrid.innerHTML = '<div class="no-products">Şu anda öne çıkan ürün bulunmamaktadır.</div>';
        return;
    }
...
function createProductCard(product) {
    ...
    console.log("ÜRÜN DETAYI:", JSON.stringify(product, null, 2));
    
    // Yıldız sayısını hesapla
    const stars = calculateStars(product.rating || 4.5);
    ...
    // Kategori adını al 
    // (Veritabanında objectId olarak tutulduğundan, kategori adı olmayabilir)
    const categoryName = product.category && typeof product.category === 'object' 
        ? product.category.name 
        : 'Bilgisayar Parçası';
    
    // Resim URL'si için farklı yaklaşımlar deneyelim
    // AÇIK VE NET bir şekilde resim URL'sini konsola yazdıralım
    let imageUrl = '/images/products/placeholder.jpg'; // Varsayılan değer
    
    console.log("1) imageUrl değeri:", product.imageUrl);
    console.log("2) images dizisi:", product.images);
    
    // imageUrl alanı varsa ve geçerliyse kullan
    if (product.imageUrl && typeof product.imageUrl === 'string' && product.imageUrl.trim() !== '') {
        imageUrl = product.imageUrl;
        console.log("3) imageUrl kullanılıyor:", imageUrl);
    }
    // Yoksa images dizisinin ilk öğesini kullan
    else if (product.images && Array.isArray(product.images) && product.images.length > 0) {
        imageUrl = product.images[0];
        console.log("4) images[0] kullanılıyor:", imageUrl);
    }
    
    console.log("5) Son resim URL'si:", imageUrl);
    
    // HTML içeriğini oluştur ve imageUrl'yi göster
    card.innerHTML = `
        <span class="featured-badge">Popüler</span>
        <div class="featured-product-image">
            <img src="${imageUrl}" alt="${product.name}" onerror="this.onerror=null; console.error('Resim yüklenemedi:', this.src); this.src='/images/products/placeholder.jpg';">
            <div class="product-overlay">
                ...
                <button class="product-action cart-btn" onclick="addToCart('${product._id}', '${product.name.replace(/'/g, "\\'")}', ${currentPrice}, '${imageUrl}')"><i class="fas fa-shopping-cart"></i></button>
            </div>
        </div>
        <div class="featured-product-info">
            ...
            <div class="featured-product-rating">
                ...
                <span class="rating-count">(${product.sales || Math.floor(Math.random() * 100 + 10)})</span>
            </div>
            <div class="featured-product-price">
                ...
                <button class="btn-add-cart" onclick="addToCart('${product._id}', '${product.name.replace(/'/g, "\\'")}', ${currentPrice}, '${imageUrl}')"><i class="fas fa-shopping-cart"></i></button>
            </div>
        </div>
    `;
...
function setupFavoriteButtons() {
    console.log("Popüler ürünler için favori butonlarını bağlama başlatılıyor...");
    
    // Önce tüm favori butonlarını seç
    const favoriteButtons = document.querySelectorAll('.popular-products .favorite-btn');
    
    console.log("Popüler ürünlerde bulunan favori buton sayısı:", favoriteButtons.length);
    
    if (favoriteButtons.length === 0) {
        console.warn("Popüler ürünlerde hiç favori butonu bulunamadı!");
        return;
    }
...
async function initializeFavorites() {
    try {
        console.log("Favoriler başlatılıyor...");
        const userInfo = JSON.parse(localStorage.getItem('userInfo') || sessionStorage.getItem('userInfo') || '{}');
        ...
        console.log("Kullanıcı favorileri yüklendi:", favorites.length, favorites.map(fav => fav._id));
        ...
        console.log("Favori butonları güncelleniyor...");
        const favoriteButtons = document.querySelectorAll('.popular-products .favorite-btn');
        console.log("Güncellenecek popüler ürün favori buton sayısı:", favoriteButtons.length);
        ...
        console.error('Favoriler başlatılırken hata oluştu:', error);
    }
}
